...
JointOperator::JointOperator()
  ...
  is_loop_(false)
{
	 std::string yaml_file="/home/robin/catkin_ws/src/dynamixel-workbench/dynamixel_workbench_operators/config/left_hand_test.yaml";
...
void ExampleActionServer::executeCB(const actionlib::SimpleActionServer<dynamixel_workbench_operators::joint_opAction>::GoalConstPtr& goal) {
	...
	else if (0==goal->input.compare("four"))
	{
			ROS_INFO("inside service call back with pose four");
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+3));  
			result_message.output = "fourth pose"; // we'll use the member variable result_, defined in our class
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
	else if (0==goal->input.compare("five"))
	{
			ROS_INFO("inside service call back with pose five");
			joint_trajectory_pub_.publish(*(jnt_tra_msg_+4));  
			result_message.output = "fifth pose"; // we'll use the member variable result_, defined in our class
			result_message.goal_stamp = goal->input;
			ROS_INFO("executed sucessfully");
        		as_.setSucceeded(result_message); 
	}
